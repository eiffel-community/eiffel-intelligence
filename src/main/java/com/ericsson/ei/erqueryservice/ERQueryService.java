...
public class ERQueryService {
	...
	/**
	 * This method is used to fetch only the upstream or downstream or both event
	 * information for ER based on the eventID and searchOption conditions.
	 *
	 * @param eventId      the id of the event.
	 * @param searchOption the SearchOption to indicate whether to search up, down
	 *                     or both ways from the eventId.
	 * @param limit        sets the limit of how many events up and/or down stream
	 *                     from the eventId to include in the result.
	 * @param levels       sets the limit of how many levels up and/or down stream
	 *                     from the eventId to include in the result.
	 * @param tree         whether or not to retain the tree structure in the
	 *                     result.
	 * @return ResponseEntity
	 * @throws PropertyNotFoundException
	 */
	public ResponseEntity getEventStreamDataById(String eventId, SearchOption searchOption, int limit, int levels,
			boolean tree) throws PropertyNotFoundException, Exception {
		try {
			ResponseEntity responseFromEr = sendRequestToER(eventId, searchOption, limit, levels, tree);
	...
	private ResponseEntity sendRequestToER(String eventId, SearchOption searchOption, int limit, int levels,
			boolean tree) throws IOException, URISyntaxException, ClientProtocolException, PropertyNotFoundException {
	...
	private void prepareRequest(String eventId, SearchOption searchOption, int limit, int levels, boolean tree)
		...
		request.setHttpMethod(HttpMethod.POST).setBaseUrl(eventRepositoryUrl).setEndpoint(eventId)
				.addParam("limit", Integer.toString(limit)).addParam("levels", Integer.toString(levels))
				.addParam("tree", Boolean.toString(tree)).addParam("shallow", Boolean.toString(shallowParameter))
