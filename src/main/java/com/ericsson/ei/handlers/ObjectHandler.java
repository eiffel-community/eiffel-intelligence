...
public class ObjectHandler {
	 ...
	 * @param event            String representation of event, used to fetch id if
	 *                         not specified
	 * @param givenId          String id is stored together with aggregated object
	 *                         in database
	 * @throws MongoDBConnectionException
	 */
	public String insertObject(String aggregatedObject, RulesObject rulesObject, String event, String givenId)
			throws MongoDBConnectionException {
		String id = givenId;
		if (id == null) {
	...
	public String insertObject(JsonNode aggregatedObject, RulesObject rulesObject, String event, String id)
			throws MongoDBConnectionException {
	 ...
	 */
	public void updateObject(String aggregatedObject, RulesObject rulesObject, String event, final String givenId) {
		String id = givenId;
		if (id == null) {
			String idRules = rulesObject.getIdRule();
			JsonNode idNode = jmespathInterface.runRuleOnEvent(idRules, event);
	...
	/**
	 * This method creates a new document containing id, aggregated object and sets
	...
	public String lockDocument(String id) {
		...
		/*
		 * As findAndModify below does not distinguish between a invalid id and locked
		 * document we need to check that the document id can be found
		 */
		if (isInvalidId(id)) {
		...
		// Checking a retryCounter to prevent a infinite loop while waiting for document
		// to unlock
		while (documentLocked && retryCounter < MAX_RETRY_COUNT) {
			try {
				JsonNode documentJson = mapper.readValue(setLock, JsonNode.class);
				Document result = mongoDbHandler.findAndModify(databaseName, aggregationsCollectionName,
						idAndNoLockCondition, documentJson.toString());
				if (result != null) {
	...
	/**
	 * This method gives the TTL (time to live) value for documents stored in the
	 * database. This value is set in application.properties when starting Eiffel
	 * Intelligence.
	 *
	...
	private boolean isInvalidId(String id) {
		...
		ArrayList<String> documentExistsCheck = mongoDbHandler.find(databaseName, aggregationsCollectionName,
				idCondition);
