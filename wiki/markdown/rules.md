...
an existing aggregated object. Should produce a JSON array, for example
    "[meta.id]"
...
Another common example is
 ...
 is equal "CAUSE". Links in an Eiffel event look like this:
...
Use a placeholder to inject the ids generated by **_IdentifyRules_**. 
The placeholder property [**mergeidmarker**](https://github.com/eiffel-community/eiffel-intelligence/blob/master/wiki/markdown/configuration.md#define-placeholder-in-rules) 
is defined in [application.properties](https://github.com/eiffel-community/eiffel-intelligence/blob/master/src/main/resources/application.properties).

If we configure the 'mergeidmarker' in application.properties:

    mergeidmarker: %myPlaceholderId%
...
event id will then be used to replace the 'mergeidmarker' in MatchIdRules:
...
Using the 'mergeidmarker' placeholder in the rules, saves the user the 
trouble of copy-pasting the same JMESPath expressions for several rules.
